...
CREATE TABLE sys.PRODUCT (
  ...
  CATEGORY_ID BIGINT(20) UNSIGNED COMMENT '카테고리 ID (CATEGORY 테이블의 CATEGORY_ID를 참조)',
  OPTION_GROUP_ID BIGINT(20) UNSIGNED COMMENT '옵션 그룹 ID (OPTION_GROUP 테이블의 OPTION_GROUP_ID를 참조)',
  STATUS ENUM('AVAILABLE', 'SOLD_OUT') DEFAULT 'AVAILABLE' COMMENT '상품 상태 (판매 가능 여부)',
  ...
  FOREIGN KEY (CATEGORY_ID) REFERENCES sys.CATEGORY(CATEGORY_ID) ON DELETE SET NULL,
  FOREIGN KEY (OPTION_GROUP_ID) REFERENCES sys.OPTION_GROUP(OPTION_GROUP_ID) ON DELETE SET NULL
